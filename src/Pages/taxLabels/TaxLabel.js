...
export default function TaxLabel() {
  const [show, setShow] = useState(false);
  const handleClose = () => setShow(false);
  const handleShow = () => setShow(true);
  const [label_name, setName] = useState("");
  const [taxLabel, setTaxLabel] = useState([]);
  const [filterTaxLabel, setFilterTaxLabel] = useState([]);
  ...
  const fetchData = async () => {
    await axios
      .get(`/ShowAllTaxLabels`, {
        headers: headers,
      })
      .then(({ data }) => {
        setTaxLabel([...data.all_tax_labels]);
        setFilterTaxLabel([...data.all_tax_labels]);
      })
      .catch(({ data }) => {
        console.log(data);
      });
  ...
  const handleSearch = (event) => {
    ...
    setFilterTaxLabel(tempLocations);
          ...
          <div className="form__card">
            <div className="form__card__title__wrap d-flex align-items-center justify-content-between">
              <div className="form__card__title__wrap__left d-flex align-items-center ">
                ...
                <div className="form__card__title border-0"> Tax Label</div>
              </div>
              ...
              <div className="form__card__flex__right">
                <div className="form__card__body__wrapper">
                  ...
                  <div className="data__table__wrapper">
                          ...
                          filterTaxLabel
                            ...
                            .map((item) => {
                              return (
                                <tr key={item.id}>
                                  <td>
                                    <p className="text-dark">{item.label_name}</p>
                                  </td>
                    ...
                    {filterTaxLabel.length > pagePostsLimit && (
                      <Pagination
                        ...
                        totalItems={filterTaxLabel.length}
                        pageNeighbours={2}
                      />
                    )}
                  </div>
